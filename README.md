...
> `groupPolicy`: `"open"` (default — respond to all messages) or `"ignore"` (skip group chats entirely). iMessage does not have an @mention system so mention-based filtering is not available.
